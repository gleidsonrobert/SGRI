...
Swiff.Uploader = new Class({
	...
	send: function(url, listParams){
		Swiff.remote(this.object, 'upload', this.instance, url, listParams);
	},
    ...
    continuar: function(name, size, url) {	
        return Swiff.remote(this.object, 'continuar', this.instance, name, size, url)
    },
	...
	register: function(callBack, url, container){
		if (!Swiff.Uploader.object || !Swiff.Uploader.loaded) {
			...
			if (!Swiff.Uploader.object) {
				...
				var obj = (container || document.body)
				obj.insertBefore(Swiff.Uploader.object, obj.firstChild);
			}
		}
