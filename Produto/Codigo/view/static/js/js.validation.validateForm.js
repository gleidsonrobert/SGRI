...
function validateElement(oElement){
  if(!oElement.bProcessed){
    ...
    oElement.onblur=function(event){
      ...
      if(sFrmFormat){
        oElement.value=formata(oElement.value,sFrmFormat);
      }      
      ...
      if(sFmtfilter){
        ...
        if(sFrmFormat){
          rex=sFrmFormat;
          rex2='';
          //So utiliza na formatação o numero de caracteres
          rex=sFrmFormat.substring(0,this.value.length);
          while(rex.indexOf('*')!=-1){
            rex=rex.replace('*','['+sFmtfilter+']');
          }
          rex='^'+rex+'$';
          rex=new RegExp(rex);
      ...
      var sgetSelectionKeyToValue=this.getAttribute("getSelectionKeyToValue");
      if(sgetSelectionKeyToValue){
        var k = sgetSelectionKeyToValue.split('|');
        getSelectionKeyToValue(oElement.id,k[0],k[1])      
      }
    ...
    oElement.onkeypress=function(event){
      ...
      var myEvent=(!window.event)? event:window.event;
      var ctrlKey=(!myEvent.ctrlKey)?keyCode.ctrlKey:myEvent.ctrlKey;
      /*Permite que as teclas precionadas com o Control pressionado funcione exemplo: Ctrl+C, Ctrl+V
      =] by Samuel*/
      pressedKey=String.fromCharCode(keyCode);
      if(ctrlKey || pressedKey.charCodeAt(0)==9 || pressedKey=='\r' || parseInt(pressedKey.charCodeAt(0))==0 || parseInt(pressedKey.charCodeAt(0))==8 || pressedKey==''){
        return true;
      }
...
function floatBrComSeparador(numero){
  ...
  for(var c=ll;c>0;c--)
  {
	...
	//Obs.: Antes o cálculo era feito com 3
	//foi mudado para 4 porque não estava setando os valores direito (Diego)
    if(((cc+1) % 7)==0)
	{
      s = '.' + s
    }
...
function formata(s,frm)
{
  
  s = removeFormatacao(s);
  for(i=0;i<s.length;i++)
  {
    sub = s.substr(i,1);
	if(sub != '(' && sub != ')') //Tive inserir esta linha Diego G. Matos
		frm = frm.replace("*",s.substr(i,1));
  }
  p = frm.indexOf('*');
  if(p!=-1)
  {
    frm = frm.substring(0,p);
  }
  return frm;
}